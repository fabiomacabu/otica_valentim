...
app.post('/api/admin/cupons/gerar', async (req, res) => {
    ...
    try {
        await pool.query(
            'INSERT INTO cupons (codigo, cliente_id, valor) VALUES ($1, $2, $3)',
            [codigo, cliente_id, valor || 10]
        );
...
app.post('/api/chat/nova-conversa', async (req, res) => {
    ...
    try {
        ...
        const conversaId = insertConversa.rows[0].id;
...
app.post('/api/chat/fechar-conversa/:conversa_id', async (req, res) => {
    try {
        await pool.query(
            'UPDATE conversas SET status = $1, updated_at = $2 WHERE id = $3',
            ['fechado', new Date().toISOString(), req.params.conversa_id]
        );
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/admin/marcar-lidas/:conversa_id', async (req, res) => {
    ...
    try {
        await pool.query(
            'UPDATE mensagens SET lida = 1 WHERE conversa_id = $1 AND remetente = $2',
            [req.params.conversa_id, 'cliente']
        );
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/admin/enviar-mensagem', async (req, res) => {
    ...
    try {
        const { rows } = await pool.query('SELECT status FROM conversas WHERE id = $1', [conversa_id]);
        if (rows.length === 0 || rows[0].status !== 'aberto') {
            return res.status(400).json({ error: 'Conversa encerrada' });
        }
        const now = new Date().toISOString();
        await pool.query(
            'INSERT INTO mensagens (conversa_id, remetente, mensagem, created_at) VALUES ($1, $2, $3, $4)',
            [conversa_id, 'admin', mensagem, now]
        );
        await pool.query('UPDATE conversas SET updated_at = $1 WHERE id = $2', [now, conversa_id]);
        res.json({ success: true });
    } catch (err) {
